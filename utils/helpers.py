...
def get_img_list(ds_name, vid_root, path):
    ...
    elif ds_name in {'Group1', 'CSLR-Group1', 'CSLR_dataset_frames'}:
        # Group1 CSV paths are directories containing extracted frames.
        # `path` is typically like "Group1/Buoi_1/Front/<sample_id>".
        frame_dir = os.path.join(vid_root, path)
        if os.path.isdir(frame_dir):
            img_path = os.path.join(frame_dir, '*')
        else:
            # Allow users to pass an explicit glob pattern in annotations.
            img_path = frame_dir
    elif ds_name == 'Full_dataset':
        # Full_dataset shares the same frame-directory layout as Group1.
        # `path` is relative to vid_root, e.g. "Group1/Buoi_2/Front/<sample_id>".
        frame_dir = os.path.join(vid_root, path)
        if os.path.isdir(frame_dir):
            img_path = os.path.join(frame_dir, '*')
        else:
            img_path = frame_dir
    elif ds_name == 'Full_dataset_real':
        # Real-world test split; paths use front_real_XXX naming but same directory layout.
        frame_dir = os.path.join(vid_root, path)
        if os.path.isdir(frame_dir):
            img_path = os.path.join(frame_dir, '*')
        else:
