...
class TemporalConv(nn.Module):
    def __init__(self, input_size, hidden_size, conv_type=2, num_classes=-1):
        ...
        self.min_input_length = self._get_min_input_length()
    ...
    def _get_min_input_length(self):
        required = 1
    ...
    def update_lgt(self, lgt):
        ...
        for ks in self.kernel_size:
            width = int(ks[1])
            if ks[0] == 'P':
                feat_len = torch.clamp(feat_len, min=width)
                feat_len = torch.div(feat_len, width, rounding_mode='floor')
            else:
                feat_len = torch.clamp(feat_len, min=width)
                feat_len -= width - 1
        return feat_len
    ...
    def forward(self, frame_feat, lgt):
        if frame_feat.size(-1) < self.min_input_length:
            pad_amount = self.min_input_length - frame_feat.size(-1)
            frame_feat = F.pad(frame_feat, (0, pad_amount))
